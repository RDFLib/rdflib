...
try:
    ...
    pytest.skip(f"skipping because {HOST} is unavailable", allow_module_level=True)
...
def test_simple_graph(get_graph):
    ...
    assert 0 == len(list(r)), "tarek doesn't like cheese"

    g2.add((tarek, likes, pizza))
    g.remove((tarek, likes, pizza))
    r = g.query("SELECT * WHERE { ?s <urn:example:likes> <urn:example:pizza> . }")
    assert 1 == len(list(r)), "only bob likes pizza"
...
def test_conjunctive_default(get_graph):
    ...
    assert (
        len(graph) == 3
    ), f"default union graph should contain three triples but contains:\n{list(graph)}"
...
def test_update_with_initns(get_graph):
    graph = get_graph
    graph.update(
...
def test_update_with_blank_node_serialize_and_parse(get_graph):
    graph = get_graph
    graph.update(
        "INSERT DATA { GRAPH <urn:example:graph> { _:blankA <urn:example:type> <urn:example:Blank> } }"
    )

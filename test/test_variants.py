...
class GraphVariants:
    ...
    def _decompose_path(cls, file_path: Path, basedir: Optional[Path]):
        ...
        match = cls._variant_regex.match(name_noext)
        if match is None:
...
EXPECTED_FAILURES = {
    ...
    ("variants/special_chars"): pytest.mark.xfail(
        reason=r"nt parser messes up when parsing '\\r'",
...
def test_variants(graph_variant: GraphVariants) -> None:
    """
    All variants of a graph are isomorphic with the first parsed variant, and
    thus eachother.
    """
    ...
    for variant_key, variant_path in graph_variant.variants.items():
        ...
        if first_graph is None:
            ...
            # GraphHelper.assert_triple_sets_equals(first_graph, graph, True)

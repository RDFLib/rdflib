...
#     """
#     Counts the number of bits set in the binary representation of ``value``.
#     """
#     return bin(value).count("1")


# class SourceSlot(enum.Flag):
#     """
#     Indicates which of the parameters ``source``, ``location``, ``file``, ``data`` are set.
#     """
#     SOURCE = enum.auto()
#     LOCATION = enum.auto()
#     FILE = enum.auto()
...
#     @classmethod
#     def combinations(cls, filter: Callable[[SourceSlot], bool]) -> Iterable[SourceSlot]:
#         """
#         Yields all combinations of the enum values.
#         """
#         for value in range(2 ** len(list(cls))):
#             enum_value = SourceSlot(value)
#             if filter(enum_value):
#                 yield enum_value
...
def call_create_input_source(
    ...
    with ExitStack() as xstack:
        ...
        # source_value = xstack.enter_context(source_type.source_from_path(input_path))

        # if SourceSlot.SOURCE & source_slot:
        #     source = xstack.enter_context(source_type.source_from_path(input_path))
        # if SourceSlot.LOCATION & source_slot:
        #     location = input_path.as_uri()
        # if SourceSlot.FILE & source_slot:
        #     if source_type is SourceType.BINARY_IO:
        #         file = xstack.enter_context(input_path.open("rb"))
        #     elif source_type is SourceType.TEXT_IO:
        #         file = xstack.enter_context(input_path.open("r"))
        #     else:
        #         raise ValueError(
        #             f"invalid source_type {source_type} for SourceSlot.FILE"
        #         )
        # if SourceSlot.DATA & source_slot:
        #     if source_type is SourceType.BYTES
...
def generate_create_input_source_cases() -> Iterable[ParameterSet]:
    ...
    """
    # formats = ["turtle", "json-ld", "xml", None]
    variants_dir = TEST_DATA_DIR.relative_to(Path.cwd()) / "variants"
    # input_path = (
    #      variants_dir / "diverse_triples.ttl"
    # )
    default_format = "turtle"
    input_paths = {
        "turtle": variants_dir / "diverse_triples.ttl",
        "json-ld": variants_dir / "diverse_triples.jsonld",
        "xml": variants_dir / "diverse_triples.xml",
        "nt": variants_dir / "diverse_triples.nt",
        "hext": variants_dir / "diverse_triples.hext",
        None: variants_dir / "diverse_triples.ttl",
    }
    ...
    def make_params(
        ...
        ).as_pytest_param(id=id)

    for (param, stream_check) in itertools.product(
        itertools.chain(SourceParam, LocationParam, FileParam, DataParam), StreamCheck
    ):
        ...
        yield from make_params(param, stream_check, expected_encoding, default_format)

    for (param, format) in itertools.product(
        LocationParam, formats.difference({default_format})
    ):
        yield from make_params(param, StreamCheck.BYTE, Holder(None), format=format)

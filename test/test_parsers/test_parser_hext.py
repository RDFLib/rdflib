...
def test_small_file_multigraph():
    ...
    for context in d.contexts():
        for triple in d.graph(context).triples((None, None, None)):
            total_triples += 1
    total_triples += len(d.default_graph)
    assert total_triples == 18


def test_small_file_multigraph_with_default_union():
    ...
    for f in [x for x in p if x.is_file()]:
        ...
        if f.name not in files_to_skip.keys():
            ...
            if not skip:
                ds2 = Dataset()
                ds2.parse(
                    data=ds1.serialize(format="hext"),
                    format="hext",
                    publicID=ds2.default_graph.identifier,
                )
                if ds2.context_aware:
                    for context in ds2.graphs():
                    ...
                    for triple in ds2.triples((None, None, None)):
                        if type(triple[2]) == Literal:
                            if triple[2].datatype == XSD.string:
                                ds2.remove((triple[0], triple[1], triple[2]))
                                ds2.add((triple[0], triple[1], Literal(str(triple[2]))))

                assert ds1.isomorphic(ds2)
            skip = False
        else:

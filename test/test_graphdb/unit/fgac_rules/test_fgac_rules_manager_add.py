...
def test_fgac_add_sends_position_param(
    client: GraphDBClient, monkeypatch: pytest.MonkeyPatch
):
...
@pytest.mark.parametrize(
    ...
    [
        ("1", "Position must be an integer."),
        (-1, "Position must be a positive integer."),
    ],
)
def test_fgac_add_rejects_invalid_position(
...
def test_fgac_add_raises_http_errors(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
...
def test_fgac_add_re_raises_other_http_errors(
    client: GraphDBClient, monkeypatch: pytest.MonkeyPatch
):
    rule = SystemAccessControlEntry(
        scope="system",

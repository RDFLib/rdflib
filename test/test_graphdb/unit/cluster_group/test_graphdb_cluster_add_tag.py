...
def test_add_tag_posts_payload_and_returns_none(
    ...
    client.cluster.add_tag("some-tag")

    mock_httpx_post.assert_called_once_with(
        "/rest/cluster/config/tag",
        json={"tag": "some-tag"},
    )
...
def test_add_tag_raises_expected_http_errors(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
...
def test_add_tag_reraises_unexpected_http_errors(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
):
...
def test_add_tag_validates_tag_type(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    tag,
):
    ...
    with pytest.raises(TypeError, match="tag must be a string"):
        client.cluster.add_tag(tag)

    mock_httpx_post.assert_not_called()


def test_add_tag_rejects_empty_tag(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
):
    mock_httpx_post = Mock()

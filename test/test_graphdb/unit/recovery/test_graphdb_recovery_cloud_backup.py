...
class _CloudBackupKwargs(t.TypedDict, total=False):
    repositories: list[str]
    backup_system_data: bool


def test_cloud_backup_sends_bucket_uri(
    ...
    client.recovery.cloud_backup("s3:///bucket/backup-name")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"backupSystemData": True, "bucketUri": "gs://bucket/backup"},
        ),
        (
            {"repositories": ["repo1"], "backup_system_data": False},
            {
                "repositories": ["repo1"],
...
def test_cloud_backup_sends_params_combinations_as_json(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    kwargs: _CloudBackupKwargs,
    ...
    client.recovery.cloud_backup("gs://bucket/backup", **kwargs)
...
def test_cloud_backup_uploads_authentication_file_bytes(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
):
    params_part = (
...
def test_cloud_backup_uploads_authentication_file_like(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
):
    params_part = (
...
def test_cloud_backup_uploads_authentication_file_path(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
...
def test_cloud_backup_validates_bucket_uri(
    client: GraphDBClient,
    invalid_bucket_uri,
):
    with pytest.raises(ValueError, match="bucket_uri must be a non-empty string"):
        client.recovery.cloud_backup(invalid_bucket_uri)
...
def test_cloud_backup_raises_value_error_for_invalid_repositories(
    client: GraphDBClient,
    invalid_repos,
):
    with pytest.raises(ValueError, match="repositories must be a list or None"):
        client.recovery.cloud_backup("gs://bucket/backup", repositories=invalid_repos)


def test_cloud_backup_raises_value_error_for_non_string_repository_ids(
    client: GraphDBClient,
):
...
def test_cloud_backup_raises_value_error_for_invalid_backup_system_data(
    client: GraphDBClient,
    invalid_system_data,
):
    with pytest.raises(ValueError, match="backup_system_data must be a bool"):
        client.recovery.cloud_backup(
            "gs://bucket/backup",
            backup_system_data=invalid_system_data,
        )


def test_cloud_backup_validates_authentication_file_type(client: GraphDBClient):
    with pytest.raises(ValueError, match="authentication_file must be"):
        client.recovery.cloud_backup(
            "gs://bucket/backup", authentication_file=object()  # type: ignore[arg-type]
        )
...
def test_cloud_backup_raises_exceptions_for_error_status_codes(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
    ...
    with pytest.raises(exception_class, match=match_message):
        client.recovery.cloud_backup("gs://bucket/backup")


@pytest.mark.parametrize("status_code", [404, 500, 502, 503])
def test_cloud_backup_reraises_other_http_errors(
    client: GraphDBClient,
    monkeypatch: pytest.MonkeyPatch,
    status_code: int,
):

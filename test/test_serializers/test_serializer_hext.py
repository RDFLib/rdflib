...
""" Note: there are four duplicate triples in the original file
...
def test_hext_dataset_linecount():
    ...
    for graph in list(d.graphs()) + [d.default_graph]:
        for triple in graph.triples((None, None, None)):
            total_triples += 1

    # count the number of serialized Hextuples, should be as per the original parsing
    lc = len(d.serialize(format="hext").splitlines())
    assert lc == total_triples
...
def test_roundtrip():
    ...
    with open(str(Path(__file__).parent.parent / "consistent_test_data/test_parser_hext_multigraph.ndjson")) as i:
        ordered_input = "".join(sorted(set(i.readlines()))).strip()

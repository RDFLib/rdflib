...
"""
    ...
    def test_base_serialize(self):
        ...
        ds = Dataset()
        ds.parse(data=s, publicID="http://example.com/", format="n3")
        assert list(g) == list(ds.graph(URIRef("http://example.com/")))
    ...
    def test_model(self):
        ...
        for s, p, o, c in ds:
            if isinstance(s, Graph):
                i += 1
        assert i == 3
        assert len(list(ds.contexts())) == 12

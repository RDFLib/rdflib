...
class NQuadsTransformer(BaseParser, Transformer):
...
nq_lark = Lark(
    open(
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), "grammar", "nquads.lark")
        ),
        "r",
    ).read(),
    parser="lalr",
...
class LarkNQuadsParser(rdflib.parser.Parser):
    format = None
    ...
    def parse(self, string_or_stream, graph=None, base="", **kwargs):
        ...
        tf = nq_lark.options.transformer
        ...
        statements = nq_lark.parse(uncommented_input)

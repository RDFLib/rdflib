...
class NTriplesStarTransformer(BaseParser, Transformer):
    ...
    def statement(self, children):
        ...
        self._call_state.graph.add((subject, predicate, object_))
        return children
...
ntstar_lark = Lark(
    open(
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), "grammar", "ntriples-star.lark")
        ),
        "r",
    ).read(),
    parser="lalr",
    transformer=NTriplesStarTransformer(),
    _plugins=lark_cython.plugins,
)


class LarkNTriplesStarParser(rdflib.parser.Parser):
    format = None
    ...
    def parse(self, string_or_stream, graph=None, base="", **kwargs):
        ...
        tf = ntstar_lark.options.transformer
        ...
        statements = ntstar_lark.parse(uncommented_input)

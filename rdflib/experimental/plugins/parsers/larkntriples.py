...
nt_lark = Lark(
    open(
        os.path.abspath(
            os.path.join(os.path.dirname(__file__), "grammar", "ntriples.lark")
        ),
        "r",
    ).read(),
    parser="lalr",
...
class LarkNTriplesParser(rdflib.parser.Parser):
    format = None
    ...
    def parse(self, string_or_stream, graph=None, base="", **kwargs):
        ...
        tf = nt_lark.options.transformer
        ...
        statements = nt_lark.parse(uncommented_input)

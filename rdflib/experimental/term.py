...
class RDFStarTriple(IdentifiedNode):
    ...
    def __new__(self, hashvalue: Optional[str]) -> "RDFStarTriple":
        self._hashvalue = hashvalue
        if hashvalue is None:
            raise Exception("a hash value is required, not None!")
        return Identifier.__new__(RDFStarTriple, hashvalue)

    def n3(self, namespace_manager=None):
        return f"_:{self}rdfstartriple"

    def __repr__(self) -> str:
        return f"""{
            'rdflib.term.RDFStarTriple' if self.__class__ is RDFStarTriple else self.__class__.__name__
            }('{str(self)}')"""

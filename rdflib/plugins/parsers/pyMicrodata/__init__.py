...
class pyMicrodata(object):
    ...
    def _generate_error_graph(self, pgraph, full_msg, uri=None):
        ...
        retval.add((bnode, ns_dc["date"],
                    Literal(datetime.datetime.utcnow().isoformat(),
                            datatype=ns_xsd["dateTime"])))
        ...
        if self.http_status is not None and self.http_status != 200:
            ...
            retval.add((htbnode, ns_ht["responseCode"], URIRef(
                    "http://www.w3.org/2006/http#%s" % self.http_status)))
    ...
    def graph_from_DOM(self, dom, graph=None):
        ...
        conversion = MicrodataConversion(dom.documentElement, graph,
                                         base=self.base)
        conversion.convert()
        return graph
    ...
    def graph_from_source(self, name, graph=None, rdfOutput=False):
        ...
        try:
            ...
            try:
                ...
                return self._generate_error_graph(graph,
                                                  "HTTP Error: %s (%s)" % (
                                                      h.http_code, h.msg),
                                                  uri=name)
            except Exception:
...
def processURI(uri, outputFormat, form):
    ...
    try:
        graph = processor.rdf_from_source(input, outputFormat, rdfOutput=(
                                                                             "forceRDFOutput" in list(
                                                                                     form.keys())) or not htmlOutput)
        if outputFormat == "n3":
        ...
        retval += "<dl>\n"
        if uri == "text:" and "text" in form and form[
                "text"].value is not None and len(form["text"].value.strip()) != 0:

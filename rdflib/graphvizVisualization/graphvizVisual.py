...
def checkPathBetween(g,node1,node2):
    ...
    if isinstance(node1,Literal) and isinstance(node2,Literal):
        queryCheck = "ASK { "+str(node1)+" ((<>|!<>)|^(<>|!<>))* "+str(node2)+" }" # Check path between node and literal 
        for i in g.query(queryCheck):
            return i
    elif isinstance(node1,Literal) and isinstance(node2,URIRef):
        queryCheck = "ASK { "+str(node1)+" ((<>|!<>)|^(<>|!<>))* <"+str(node2)+"> }" # Check path between node and node
        for i in g.query(queryCheck):
            return i
    elif isinstance(node1,URIRef) and isinstance(node2,Literal):
        queryCheck = "ASK { <"+str(node1)+"> ((<>|!<>)|^(<>|!<>))* "+str(node2)+" }" # Check path between literal and node
        for i in g.query(queryCheck):
            return i
    elif isinstance(node1,URIRef) and isinstance(node2,URIRef):
        queryCheck = "ASK { <"+str(node1)+"> ((<>|!<>)|^(<>|!<>))* <"+str(node2)+"> }" # Check path between literal and literal
        for i in g.query(queryCheck):
            return i

def getPropertiesPath(g,node1,node2):
    """ Code to get properties in the path of 2 nodes"""
    if(checkPathBetween(g,node1,node2)): # Query to retrieve property path
        queryProperties = """ SELECT ?temp ?prop ?temp2 WHERE{
                            {
                                <"""+str(node1)+"""> (<>|!<>)* ?temp .
                                ?temp ?prop ?temp2 .
                                ?temp2 (<>|!<>)* <"""+str(node2)+""">
...
def showPropertiesPath(g,node1,node2,filename,shortMode = False,format1="pdf"):
    ...
    listNodes = getPropertiesPath(g,node1,node2) # Retrieve all the paths
    if shortMode: # If there in short node
        for node in nodes: # For nodes in graph
            dot.node(str(nodes[node][1]),label=nodes[node][2],tooltip=node,shape=shapeDict[nodes[node][0]],style="filled", fillcolor=colorDict[nodes[node][0]]) # Add short name nodes in the dot graph
        ...
        for edge in listNodes:
            dot.edge(str(nodes[edge[0]][1]),str(nodes[edge[2]][1]),color="red",tooltip=edge[1],label=str(g.compute_qname(edge[1],generate=True)[2])) # Add edges in dot graph
    else:
        for node in nodes: # For nodes in graph
            dot.node(str(nodes[node][1]),label=node,tooltip=node,shape=shapeDict[nodes[node][0]],style="filled", fillcolor=colorDict[nodes[node][0]]) # Add nodes in the dot graph
        ...
        for edge in listNodes:
            dot.edge(str(nodes[edge[0]][1]),str(nodes[edge[2]][1]),color="red",tooltip=edge[1],label=edge[1]) # Add edges in dot graph

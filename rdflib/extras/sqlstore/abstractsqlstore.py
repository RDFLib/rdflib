...
from rdflib.plugins.stores.regexmatching import (
    PYTHON_REGEX,
    REGEXTerm)
...
class SQLGenerator:
    def executeSQL(self, cursor, qStr, params=None, paramList=False):
        ...
        if not params:
            ...
            params = tuple([not isinstance(item, int)
                            and u"'%s'" % item or item
                                for item in params])
    ...
    def buildLiteralTripleSQLCommand(self,
                                     subject,
                                     predicate,
                                     obj,
                                     context,
    ...
    def buildTripleSQLCommand(self,
                              subject,
                              predicate,
                              obj,
                              context,
        ...
        """
        stmt_table = quoted \
            and "%s_quoted_statements" % storeId \
    ...
    def buildClause(self,
        ...
        if typeTable:
            ...
            if clauseParts is not None:
                ...
                parameters.extend(
                    [param for param in clauseParts[-1] if param is not None])
    ...
    # Stubs for Clause Functions that are overridden by specific
    # implementations (MySQL vs SQLite for instance)
...
class AbstractSQLStore(Store, SQLGenerator):
    ...
    # Stubs to be overidden as required
            ...
            if isinstance(context, QuotedGraph) or predicate != RDF.type:
                ...
                if isinstance(obj, Literal):
                    ...
                    cmd, params = self.buildTripleSQLCommand(
                        subject, predicate, obj, context,
                        self._internedId, isinstance(context, QuotedGraph))
    ...
    def triples(self, (subject, predicate, obj), context=None):
        ...
        rt = c.fetchone()
        while rt:
            s, p, o, (graphKlass, idKlass,
                      graphId) = extractTriple(rt, self, context)
            ...
            while sameTriple:
                s2, p2, o2, (graphKlass, idKlass,
                             graphId) = extractTriple(next, self, context)
    ...
    def __repr__(self):
        ...
        try:
            return "<Partitioned SQL N3 Store: " + \
                "%s contexts, " % len([c for c in self.contexts()]) + \
                "%s classification assertions, " % typeLen + \
                "%s quoted statements, " % quotedLen + \
                "%s property/value assertions, " % literalLen + \
                "and %s other assertions>" % assertedLen
        except Exception:
    ...
    def bind(self, prefix, namespace):
        ...
        try:
            c.execute(
                "INSERT INTO %s_namespace_binds" % self._internedId +
                " (prefix,uri) VALUES ('%s', '%s')" % (prefix, namespace))
        except:
            pass
        c.close()

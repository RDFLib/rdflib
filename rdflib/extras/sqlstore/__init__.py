"""
``Context-aware``: An RDF store capable of storing statements within
contexts is considered to be context-aware. In essence, such a store
is able to partition the RDF model it represents into individual,
named and addressable sub-graphs.

(Relevant Notation3 reference regarding formulas, quoted statements, and
such: http://www.w3.org/DesignIssues/Notation3.html)

``Formula-aware``: An RDF store that is capable of distinguishing between
statements which are asserted and statements which are quoted is considered
to be formula-aware.
...
identifiers. These distinguishing identifiers are not necessarily required
to be RDF identifiers, they may be, for example, an RDF identifier that has
been normalized (with SHA1/MD5 perhaps) for database naming purposes.

Conjunctive Query: Any query that doesn't limit the store to search only
within a named context. Such a query expects a context-aware store to search
the entire asserted universe (the Conjunctive Graph). A formula-aware store
is expected not to include quoted statements when matching such a query.
"""
...
class Store(object):
    ...
    def open(self, configuration, create=False):
        ...
        store.  This should return one of VALID_STORE, CORRUPTED_STORE,
        or NO_STORE
        """
        return UNKNOWN
